...
def parse_simple_condition(cond: str) -> Tuple[str, int]:
    """
    Accepts: '>=7', '<=5', '2d6>=9'
    Returns (op, target_int)
    """
    cond = str(cond).strip()
    cond = cond.replace("2d6", "").strip()
...
def eval_condition(cond: Optional[str], ctx: Dict[str, Any]) -> bool:
    ...
    Supported:
      - blank / NaN -> True
      - 'var.type==item' where var is in ctx and ctx[var] is a dict with "type"
    ...
    # direct boolean flag
    if cond in ctx and isinstance(ctx[cond], bool):
        return ctx[cond]

    m = re.match(r"^([a-zA-Z_]\w*)\.type\s*==\s*([a-zA-Z_]\w*)$", cond)
    if m:
        var, typ = m.group(1), m.group(2)
        card = ctx.get(var)
        if not isinstance(card, dict):
            return False
        return str(card.get("type", "")).lower() == typ.lower()
...
def resolve_effect(step: EffectStep,
    ...
    # Roll gate (used by hero effects mainly; monster attack rolls handled elsewhere)
    ...
    if ek in ("draw_card", "draw_cards"):
    ...
    elif ek in ("discard_card", "discard_cards"):
        n = step.amount if step.amount is not None else 1
        ...
        if cid not in state.players[pid].hand:
            ...
            return
        # MVP: free play, but still challengeable (it *is* a play)
        play_card_from_hand(
            state=state,
            engine=engine,
            pid=pid,
    ...
    elif ek == "sacrifice_hero":
        victim_pid = ctx.get("target_pid", pid)  # default: self
    ...
    elif ek == "protection_from_steal":
        ctx["protect.steal"] = True
    elif ek == "protection_from_destroy":
        ctx["protect.destroy"] = True
    elif ek == "protection_from_challenge":
